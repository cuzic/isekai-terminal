...
fun ProfileListScreen(
    ...
    val profiles = remember { mutableStateListOf<ConnectionProfile>() }
    var passwordTarget by remember { mutableStateOf<ConnectionProfile?>(null) }
    var deleteTarget by remember { mutableStateOf<ConnectionProfile?>(null) }

    suspend fun refresh() {
        val list = withContext(Dispatchers.IO) { Repositories.profiles.getAll() }
        profiles.clear()
        ...
        AlertDialog(
            ...
            confirmButton = {
                TextButton(onClick = {
                    deleteTarget = null
                    scope.launch {
                        RemoteLogger.i("TsshProfile", "deleted profile id=${target.id} '${target.label}'")
                        withContext(Dispatchers.IO) { Repositories.profiles.delete(target) }
                        refresh()
                    }
                }) { Text("削除") }
            },
