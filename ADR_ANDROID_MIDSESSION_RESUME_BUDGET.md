...
**実装時に判明した注意点(スパイク計測基盤の実装レビューで発覚)**:

- **network wakeで打ち切った回はリトライ回数(`REATTACH_MAX_RETRIES`)を
  消費しない設計にする必要がある**: 単純に「打ち切ったら次のattemptへ即進む」
  だけだと、ネットワークパス変化のイベントが連続する状況
  (`NetworkPathMonitor`はDIRECT/TAILSCALEの2本の`NetworkRequest`を持ち、
  1回のWi-Fi⇔セルラーハンドオーバーで複数回発火しうる。加えて全ペインに
  ファンアウトされる)で、`reattach_fn`のタイムアウト待ち(最悪15秒級)だけで
  5回の試行回数を高速に焼き切ってしまい、**むしろ退行する**(「Wi-Fiに戻った
  瞬間に繋がる」の狙いと正反対に、Wi-Fiに戻った瞬間に諦めるようになる)。
  network wake由来の打ち切りは、前回までの失敗と独立した新しい機会として
  扱い、別枠(上限付き、無制限の無限ループを防ぐため)でカウントすること。
  最小フロア(即座にゼロ遅延でリトライしない)も併せて必要。
- **STUN P2P経路(`isekai_stun_p2p_transport.rs`)は本タスクの対象外として
  明示的に残す**: このタスクが実装するreattachは`peer_addr`(穴あけ済みNAT
  マッピング)を接続時にキャプチャして固定するため、真にクライアント側の
  ネットワークパスが変わった場合、そのマッピングは既に死んでおり、
  同じ`peer_addr`へのreattachは原理的に成功しない
  (`isekai_link_relay_transport.rs`のコメントが「relayは常時経路に残るため
