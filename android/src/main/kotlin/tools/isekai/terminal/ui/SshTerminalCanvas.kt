...
/**
 * セル寸法(cellW/cellH)ごとにフォントサイズ・ベースライン計算をキャッシュする。
 * `measureText`/`fontMetrics` は cellW/cellH が変わらない限り結果が変わらないが、
 * 以前は Canvas が再描画されるたび(＝Rust から ScreenUpdate 通知が来るたび)無条件に
 * 再計算していた。
 */
private class FontFitCache {
    var cellW = -1f
    var cellH = -1f
    var baseline = 0f
}
...
/**
 * 直近に描画したセル格子(背景+文字+カーソル)を off-screen Bitmap にキャッシュする。
 *
 ...
 * テーマ背景色が前回描画時と変わっていなければセル全走査(`drawRect`/`drawText`)を
 * スキップし、キャッシュ済み Bitmap を `drawImage` で貼り直すだけにする。
 *
 * 選択ハイライトとカーソルはこの Bitmap キャッシュの対象外とし、毎フレーム軽量に
 * 描画し直す(選択ハイライトは choice のあった行だけ、カーソルは1セル分の矩形のみ)。
 */
private class GridRenderCache {
    var bitmap: Bitmap? = null
    var renderedUpdate: ScreenUpdate? = null
    var renderedCellW = -1f
    var renderedCellH = -1f
    var renderedThemeBg = 0
}
...
fun SshTerminalCanvas(
    ...
    Canvas(modifier = modifier.background(theme.background)) {
        ...
        // (cellW/cellH が変わらない限りキャッシュを使い回す)
        if (fontFit.cellW != cellW || fontFit.cellH != cellH) {
        ...
        if (bmp == null || bmp.width != pixelW || bmp.height != pixelH) {
            ...
            gridCache.renderedUpdate = null
        }

        val needsRender = gridCache.renderedUpdate !== update ||
            gridCache.renderedCellW != cellW ||
            gridCache.renderedCellH != cellH ||
            gridCache.renderedThemeBg != themeBgArgb
        if (needsRender) {
            ...
            if (cx < size.width && cy < size.height) {
                ...
                bitmapCanvas.drawRect(cx, cy, cx + cellW, cy + cellH, bgPaint)
            }

            gridCache.renderedUpdate = update
            gridCache.renderedCellW = cellW
            gridCache.renderedCellH = cellH
            gridCache.renderedThemeBg = themeBgArgb
        }
