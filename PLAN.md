...
- 新規`ios/Sources/TsshCore/TerminalSelection.swift`: `CellPos`/`SelectionRange`
...
### iOS Linux CI: TsshCoreLogicの切り出し(2026-07-05、「Linux側テストを充実させて」指示を受けて着手)
...
`ios/Sources/TsshCore`から UIKit/SwiftUI/GRDB/Keychain に依存しない部分を
`ios/Sources/TsshCoreLogic`という新ターゲットへ切り出した。
...
(GRDBの`Snippet`型に依存するオーバーロードだけは`TsshCore`側の`extension`として残した)。
UniFFI生成物(`generated/`)も`TsshCoreLogic`側へ移した(内容はホストOS非依存で
Linux/macOSどちらで生成しても同一と確認済み)。

**Linux上でのFFIリンク方式**: XCFrameworkはApple専用パッケージング形式でLinuxでは
使えないため、`ios/Sources/TsshCoreFFILinux`という`systemLibrary`ターゲットを新設し、
`rust-core/scripts/build-linux-swift-ffi.sh`がネイティブビルドした`libtssh_core.so`
(`cargo build -p tssh-core`、クロスコンパイル不要)へ直接リンクする。uniffi-bindgenが
...
するため、`TsshCore`/`TsshCoreTests`(UIKit/GRDB/CryptoKit依存)がマニフェストに
存在するだけでLinux上の`swift test`がそれらのコンパイルに巻き込まれて失敗する。
`ios/Package.swift`全体を`#if os(Linux)`で分岐させ、Linux上では`TsshCoreFFIBinary`
(`.binaryTarget`)・`TsshCore`・`TsshCoreTests`自体をマニフェストから丸ごと除外する
...
Phase 1G-2(#54)が実装され(`46d3881`→`1322c12`)、未コミットの`import TsshCoreLogic`
挿入を「紛れ込んだ不要import」と誤解してその2ファイル(`TerminalTabsHostView.swift`/
`TerminalView.swift`)から一旦削除するfixコミットが入った。`TsshCoreLogic`が正式な
