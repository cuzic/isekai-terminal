...
///
/// Y-P0時点では既存4トグルの移設のみで新機能は足していなかったが、Y-P1(#7)で
/// ホスト鍵自動信頼トグルを追加した。今後 #3(フォントインポート入口、Y-P4)・
...
public struct SettingsView: View {
    ...
    /// Y-P1(#7): 既定OFF。`TerminalSessionController.onHostKey`が読む。
    @AppStorage(AppSettingsKeys.autoTrustNewHostKeys) private var autoTrustNewHostKeys = false
    ...
    public var body: some View {
        NavigationStack {
            List {
                ...
                Section {
                    toggleRow(baseLabel: "新規ホスト鍵を自動信頼", isOn: $autoTrustNewHostKeys, identifier: "autoTrustNewHostKeysToggle")
                } footer: {
                    Text("オンにすると初回接続時のfingerprint確認を省略します。信頼できるネットワークでのみ使用してください。既知ホストのfingerprint不一致は、この設定に関わらず常に拒否されます。")
                }
            }
            .navigationTitle("設定")
