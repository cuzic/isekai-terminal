...
xcodebuild test -scheme TsshCore-Package -destination 'platform=iOS Simulator,name=iPhone 15'
```

`-scheme TsshCore-Package` は `Package.swift` から自動生成される、全ターゲット(`TsshCore`/
`TsshCoreLogic`とそれぞれのテストターゲット)を束ねるスキームです。`TsshCoreLogic`分離
(Phase Y、`TsshCoreLogic`分離の設計判断の節参照)以降、単体の`TsshCore`スキームには
