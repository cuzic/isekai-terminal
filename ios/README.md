...
xcodebuild test -scheme IsekaiTerminalCore -destination 'platform=iOS Simulator,name=iPhone 15'
```

`-scheme IsekaiTerminalCore` は `Package.swift` から自動生成されるスキームで、CI(`macos-26`)では
そのまま動作することを確認済みです。
