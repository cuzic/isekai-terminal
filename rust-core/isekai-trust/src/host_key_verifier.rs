//! A [`russh_stream_session::HostKeyVerifier`] backed by this crate's
//! [`SshHostKeyTrustStore`] (`known_ssh_hosts.toml`) — the TOFU host-key
//! check that stands in for `ssh(1)`'s own `known_hosts` prompt, shared by
//! every native (non-`ssh(1)`-subprocess) SSH path: `isekai-ssh`'s own
//! connect path and `isekai-bootstrap`'s `RusshBackend`.
//!
...
//!   re-deployed has the exact same recovery path `ssh(1)`'s own
//!   `~/.ssh/known_hosts` mismatch has always forced: remove the stale entry
//!   and reconnect. Automating that removal would defeat the point of
//!   pinning.
...
/// TOFU semantics (see the module docs).
///
/// [`SshHostKeyTrustStore`]: crate::SshHostKeyTrustStore
pub struct FileBackedHostKeyVerifier {
    store_path: PathBuf,
    host_port: String,
    ...
    /// Prefix for this verifier's log lines (e.g. `"isekai-ssh"` vs.
    /// `"isekai-bootstrap"`), so a log reader can tell which caller's TOFU
    /// check emitted a mismatch/failure message.
...
impl FileBackedHostKeyVerifier {
    pub fn new(
        ...
        Self { store_path, host_port, confirm_new_host, log_context }
    }
}
...
impl HostKeyVerifier for FileBackedHostKeyVerifier {
    async fn verify(&self, fingerprint: &str) -> bool {
        ...
        // callback. Must never happen while holding the store lock — the
        // lock key is the whole `known_ssh_hosts` store, not scoped per
        // host, so a human staring at one new-host prompt would otherwise
        // block every *other* tab's host-key checks too, even for completely
        // unrelated, already-known hosts.
        ...
        // blindly inserting — another tab connecting to the same brand-new
        // host may have raced us while we were waiting on the user, in which
        // case this reuses the exact same decision logic (accept-and-refresh
        // if it now matches, reject if it now doesn't) instead of
        // overwriting whatever that other tab just decided.
...
impl FileBackedHostKeyVerifier {
    ...
    async fn resolve_locked(&self, fingerprint: &str, insert_if_unknown: bool) -> Resolved {
        ...
        // can block for an arbitrary time on the cross-process lock — run it
        // on a blocking-pool thread so it never stalls this tokio runtime's
        // async workers.
...
mod tests {
    use super::*;
    use crate::SshHostKeyTrustStore;
    ...
    /// host, so `confirm_new_host` must run outside it.
    ...
    async fn a_pending_confirmation_for_one_host_does_not_block_a_different_known_host() {
        ...
        crate::save_ssh_host_key_trust_store(&store_path, &seed).unwrap();
