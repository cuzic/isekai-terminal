...
//! are out of scope for this module and land in a later change (S-4a).
...
mod tests {
    ...
    fn rejects_unknown_frame_type() {
        // 0x03 is reserved for RESUME (HELPER_PROTOCOL.md §4/§7.3), which is
        // intentionally not implemented by this crate yet (see S-4a).
        let err = identify_frame_type(0x03).unwrap_err();
        assert_eq!(err, ProtocolError::UnknownFrameType(0x03));
    }
}
