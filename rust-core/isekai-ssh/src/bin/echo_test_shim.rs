...
//! line for each of `ISEKAI_INTENT_ID`/`ISEKAI_PIPE_RUNTIME_DIR`/
//! `ISEKAI_PIPE_LOG_FILE` (verifying a caller like `spawn_isekai_pipe_connect`
//! actually set the env vars it claims to, without needing a real
//! `isekai-pipe` binary or mutating this test process's own environment — no
//! `std::env::set_var` anywhere, since that's process-global and races
...
fn main() {
    ...
    for name in ["ISEKAI_INTENT_ID", "ISEKAI_PIPE_RUNTIME_DIR", "ISEKAI_PIPE_LOG_FILE"] {
