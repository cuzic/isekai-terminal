...
**現時点の Windows ネイティブ経路では、`#@isekai ctl-socket yes` はまだ配線されていない**
(`native/connect.rs` はリモートフォワードの要求を出しておらず、ctl-socket 用のローカル
受け口も張らない。設定に書いても native 経路では何も起きない)。この機能——リモートの
対話シェルから `isekai-pipe ctl title "<text>"` / `isekai-pipe ctl clip push` を実行して
ローカルのタブのタイトル・クリップボードへ反映する(`ISEKAI_PIPE_DESIGN.md` §8 Epic M)
——は、現状 Unix の実 `ssh(1)` 経路(`ctl_forward.rs` の `-R` リモートフォワード)でのみ
有効。native 経路への named pipe 経由の移設(計画書 M5)は進行中で、完了するまで Windows
native では無効。
