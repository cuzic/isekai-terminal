...
```
--isekai-tty              : 下記の規則で名前を自動導出して使う
--isekai-tty=<name>       : デーモン名を明示指定(同じホストに複数の名前付きセッションを持てる)
```

`--isekai-tty`(名前省略)の自動導出規則:

**「そのタブ/ペインが生きている間ずっと同じ値で、`isekai-ssh` プロセス自体を`kill`して
立て直しても不変」な環境変数を、以下の優先順位で探して見つかった最初のものを使う**
(`<prefix>-<値>` という名前になる)。これにより、同じタブから `isekai-ssh --isekai-tty
myhost` を打ち直すたびに常に**同じリモートシェル**に戻り、かつ同じホストへ繋いだ**別の
タブ**は自動的に**別のデーモン**になる——ユーザーが名前を考える必要も、生きている
セッション一覧から選ぶピッカーも不要(検討はしたが、この方式で決定的に解決できるため
スコープ外にした)。
...
| 7 | `STY` | GNU Screen(セッション単位、pane単位より粒度が粗い) |

意図的に**pty デバイスパス(`tty(1)`/`$SSH_TTY`)は使わない**: pty番号はタブを閉じると
カーネルに回収され、後で無関係な別タブが同じ番号を再取得すると、閉じたはずのタブの
(まだ生きている)古いデーモンに黙って相乗りしてしまう。上記はどれもアプリ自身が発行する
使い回されないカウンタ/UUIDなので、この問題が起きない。

上記のどの環境変数も対応しない素のターミナル(マルチプレクサなしの gnome-terminal /
Terminal.app 等)では、`.bashrc`/`.zshrc` に以下を1行足すことで同じ仕組みに乗れる:

```bash
if [ -z "$ISEKAI_TTY_SESSION" ]; then
...
どの環境変数も見つからなければ `isekai-<profile>`(ホストごとに1つの既定デーモン、
タブの区別なし)にフォールバックする——この機能導入前からの挙動そのまま。
