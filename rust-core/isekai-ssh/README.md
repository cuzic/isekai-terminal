...
- **`--isekai-helper-binary`/`--helper-binary` を省略した際の自動ダウンロードは、実際には
  ...
  (署名鍵の生成・保管方針が未確定のため意図的に保留中)。ダウンロード先の URL 構築・
  アーキ検出・キャッシュの仕組み自体は実装済みだが、実在する release asset が無いので
  404 になり、既存の「`--helper-binary` を渡すか `init` を実行してください」というエラーに
  フォールバックする。実際にリリースを公開する際は `isekai-pipe-<arch>-unknown-linux-musl`
  (+任意で `.sha256` サイドカー)という asset 名の規約に従う必要がある。
- リリース成果物の署名検証(`--helper-manifest`/`--trusted-release-key`)は実装済みだが
  opt-in(デフォルト無効)。信頼できるバイナリかどうかは既定では `init`/自動 bootstrap 時に
  アップロードした実体の sha256 を記憶しておくことだけで担保している
  (`update_policy = exact-digest-only`)。署名鍵自体の生成・配布はまだ行っていない。
