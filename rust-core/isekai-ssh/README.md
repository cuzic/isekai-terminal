...
- **Windows Terminal 上では `$WT_SESSION`(そのタブ/ペインが生きている間ずっと同じ値の
  GUID、中身の子プロセスを`kill`して立て直しても不変)から `wt-<WT_SESSION>` を導出する**。
  これにより、同じタブから `isekai-ssh --isekai-tty myhost` を打ち直すたびに常に**同じ
  リモートシェル**に戻り、かつ同じホストへ繋いだ**別のタブ**は自動的に**別のデーモン**に
  なる——ユーザーが名前を考える必要も、生きているセッション一覧から選ぶピッカーも不要。
  (他ターミナル・tmux 越しなど `$WT_SESSION` に相当する「タブが生きている間ずっと不変な
  ID」を子プロセスが読める仕組みが無い環境向けのピッカーは、意図的にスコープ外)。
- それ以外の環境では `isekai-<profile>`(ホストごとに1つの既定デーモン、タブの区別なし)
  にフォールバックする——`$WT_SESSION` 導入前の挙動そのまま。
