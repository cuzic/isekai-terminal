...
**A. relay 経由(推奨・既定。NAT 越え・ローミング耐性あり)**

```bash
# 1. relay の JWT を取得(既に持っているなら省略可)
