//! Binding a UDP socket to a specific physical network interface, and
//! dialing through it. Originally `isekai-transport::physical_interface`
//! (isekai-terminal's PC/CLI-side counterpart of `AnyMuxRebinder::
//! rebind_socket`); moved here because it has no dependency on
//! `isekai-transport`/`isekai-protocol` — it only needs `quicmux` itself
//! (for [`AnyMuxFactory`]/[`RemoteSpec`]/[`MuxError`]) and the vendored
//! `quicsock` crate, exactly the kind of "pure, potentially-reusable
//! mechanic" this crate's docs already say belongs here (see
//! [`crate::race::race_with_stagger`], moved out of `isekai-transport` for
//! the same reason).
//!
//! This is a thin wrapper over `quicsock` — see that crate's own docs for
//! the per-platform mechanism (`SO_BINDTOIFINDEX`/`IP_BOUND_IF`/
//! `IP_UNICAST_IF`) and its important Android caveat (a plain
//! interface-index bind does not reliably route traffic through the
//! requested interface on Android app/framework environments — Android's
//! own rebind path does not go through this module at all, it imports an fd
//! already bound via `android.net.Network.bindSocket()` on the Kotlin/JNI
//! side and hands it straight to `rebind_socket`, matching `quicsock`'s own
//! recommendation).
...
// Re-exported (not just `InterfaceIndex`) so callers can enumerate
// interfaces via `quicsock::discovery` without adding their own direct
// dependency on `quicsock`.
pub use quicsock;
pub use quicsock::InterfaceIndex;

/// Binds a UDP socket restricted to `interface` at `local_addr`, ready to
/// pass to [`crate::AnyMuxRebinder::rebind_socket`]. `quicsock` itself
/// returns a [`socket2::Socket`] (implementation-agnostic — see its own
/// module docs); this converts it to the plain [`std::net::UdpSocket`]
...
/// wired interfaces this module's other dial paths were originally written
/// against.
...
/// all race for the low end of the range. Mirrors `noq_backend`'s own
/// (private) `candidate_ports` helper — that one drives
...
fn bind_physical_interface_with_port_range(
    ...
    Err(MuxError::Bind { addr: unspecified, source: last_err.unwrap_or_else(|| io::Error::new(io::ErrorKind::InvalidInput, "empty port range")) })
}
...
/// dial" step shared by `isekai-transport`'s `WarmStandby`/
/// `connect_dual_path` and (generically) [`crate::warm_standby::WarmStandby`]:
/// all three need exactly the same bind-or-wrap-then-connect sequence, just
...
/// same way [`BindSpec::with_port_range`] does for this crate's other dial
/// paths — `None` binds an OS-assigned ephemeral port, as before.
pub async fn connect_via_interface(
    factory: &AnyMuxFactory,
    interface: Option<InterfaceIndex>,
    remote: RemoteSpec,
    port_range: Option<(u16, u16)>,
) -> Result<AnyMuxConnection, MuxError> {
    let unspecified = unspecified_addr_for(remote.addr);
    let endpoint = match interface {
        None => {
            let bind = BindSpec { local_addr: unspecified, port_range };
            factory.create_endpoint(bind).await?
        }
        Some(interface) => {
            ...
            let tokio_socket = tokio::net::UdpSocket::from_std(std_socket).map_err(|e| MuxError::SocketSetup(e.to_string()))?;
            factory.wrap_bound_socket(tokio_socket).await?
        }
    };
...
mod tests {
    ...
    use crate::{AnyMuxListener, MuxClientConfig, MuxServerConfig};
    ...
    // through it. Unix's `SO_BINDTOIFINDEX`/`IP_BOUND_IF` reject a
    // nonexistent index immediately, which is what this test actually
    // verifies there.
    #[test]
    #[cfg(not(windows))]
    ...
    const SNI: &str = "quicmux-iface-dial-test.local";
    const TEST_ALPN: &[u8] = b"quicmux-iface-dial-test";
    ...
    fn build_server_config() -> (MuxServerConfig, String) {
        ...
        let config = MuxServerConfig {
            ...
            exporter_label: b"quicmux-iface-dial-test-exporter".to_vec(),
    ...
    async fn spawn_listener(bind_addr: SocketAddr) -> (SocketAddr, String) {
        ...
        let listener = AnyMuxListener::bind_noq(server_config, BindSpec { local_addr: bind_addr, port_range: None }).await.unwrap();
    ...
    /// specifically when the bind fails with `MuxError::EndpointSetup` — a
    /// known CI-runner-specific IPv6 quirk on `test-windows`, not a genuine
    /// Windows/`noq` limitation (confirmed directly on a real Windows
    /// machine). Any *other* error still panics — this is deliberately
    ...
    async fn try_spawn_listener(bind_addr: SocketAddr) -> Option<(SocketAddr, String)> {
        let (server_config, cert_sha256_hex) = build_server_config();
        let listener = match AnyMuxListener::bind_noq(server_config, BindSpec { local_addr: bind_addr, port_range: None }).await {
    ...
    async fn connect_via_interface_dials_an_ipv6_remote() {
        ...
        let remote = RemoteSpec { addr, server_name: SNI.to_string(), cert_sha256_hex };
