...
pub(crate) async fn push_ctl_socket_to_tmux<R: RemoteTmuxCommandRunner>(
    ...
    log::error!("ISEKAI_DEBUG_MARKER: push_ctl_socket_to_tmux app_pane={app_pane_id:?} locator={locator:?} ctl_socket_path={ctl_socket_path}");
    match &locator {
        Some(locator) => {
            let resolver = TmuxLocatorResolver::new(runner);
            let result = resolver.push_ctl_socket_path(locator, ctl_socket_path).await;
            log::error!("ISEKAI_DEBUG_MARKER: push_ctl_socket_to_tmux resolve+set-option result={result:?}");
