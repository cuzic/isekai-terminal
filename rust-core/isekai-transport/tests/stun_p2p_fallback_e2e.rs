...
use isekai_transport::{connect_stun_p2p_with_fallback, SequentialStunCandidate, SequentialStunConnectError, StunP2pTarget};
...
async fn first_candidate_unreachable_stun_falls_back_to_second_and_succeeds() {
    ...
    let (mut conn, winning_stun_server) =
        tokio::time::timeout(Duration::from_secs(10), connect_stun_p2p_with_fallback(&target, &candidates))
            .await
...
async fn a_terminal_failure_on_the_first_candidate_stops_without_trying_the_second() {
    ...
    match tokio::time::timeout(Duration::from_secs(10), connect_stun_p2p_with_fallback(&target, &candidates)).await {
...
async fn no_candidates_is_a_caller_error() {
    ...
    match connect_stun_p2p_with_fallback(&target, &[]).await {
