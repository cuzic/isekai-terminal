...
mod tests {
    ...
    async fn notify_unless_deferred_does_not_escalate_a_waiting_tab_but_still_self_corrects() {
        ...
        // no-op transition writes nothing at all, so the tty should still
        // simply end with the waiting color `stop_deferred` already painted.
        tokio::time::sleep(Duration::from_millis(40)).await;
        let tty_contents = std::fs::read_to_string(&tty_path).unwrap();
        assert!(
            tty_contents.trim_end().ends_with("\x1b]4;264;rgb:00/60/a0\x1b\\"),
