...
#
# 出力: ../ios/Sources/TsshCoreLogic/generated/ 以下に生成物と .sha256
# (TsshCoreLogic/TsshCore双方が参照する唯一のコピー。中身はホストOS非依存で
#  Linux/macOSどちらで生成しても同一。Linux向けsystemLibraryターゲット
#  `TsshCoreFFILinux`はここから派生させる、build-linux-swift-ffi.sh参照)
set -euo pipefail
...
OUT_DIR="../ios/Sources/TsshCoreLogic/generated"
rm -rf "$OUT_DIR"
mkdir -p "$OUT_DIR"
