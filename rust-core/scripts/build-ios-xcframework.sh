#!/usr/bin/env bash
# isekai-terminal-core を iOS 向けにクロスコンパイルし、XCFramework としてパッケージングする。
#
...
for t in "$TARGET_DEVICE" "${TARGETS_SIM[@]}"; do
    echo "=== building isekai-terminal-core for $t ==="
    cargo build --release --target "$t" -p isekai-terminal-core
done
...
mkdir -p "$SIM_FAT_DIR"
lipo -create \
    "target/aarch64-apple-ios-sim/release/libisekai_terminal_core.a" \
    "target/x86_64-apple-ios/release/libisekai_terminal_core.a" \
    -output "$SIM_FAT_DIR/libisekai_terminal_core.a"

# xcodebuild -create-xcframework の -headers ディレクトリは `module.modulemap` という
# ファイル名を期待する。uniffi-bindgen の実際の出力ファイル名は `isekai_terminal_coreFFI.modulemap`
# （module宣言自体は `module isekai_terminal_coreFFI { ... }` のままでよく、ファイル名だけの問題）。
HEADERS_DIR="target/ios-xcframework-headers"
rm -rf "$HEADERS_DIR"
mkdir -p "$HEADERS_DIR"
cp "$GENERATED_DIR/isekai_terminal_coreFFI.h" "$HEADERS_DIR/"
cp "$GENERATED_DIR/isekai_terminal_coreFFI.modulemap" "$HEADERS_DIR/module.modulemap"

OUT_XCFRAMEWORK="../ios/Frameworks/IsekaiTerminalCoreFFI.xcframework"
rm -rf "$OUT_XCFRAMEWORK"
xcodebuild -create-xcframework \
    -library "target/$TARGET_DEVICE/release/libisekai_terminal_core.a" -headers "$HEADERS_DIR" \
    -library "$SIM_FAT_DIR/libisekai_terminal_core.a" -headers "$HEADERS_DIR" \
