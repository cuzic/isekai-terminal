...
- **スコープ外(意図的に対象外)**: QUICレベルのresume/multipath/rebind自体の変更なし
  (初回attach段階の分類のみ)。新規ホストのTOFU確認は変更なし(必須のまま)。新しい
  `#@isekai`ディレクティブは追加していない。キャッシュされたアドレス自体の古さ(STUN
  観測アドレス・direct-by-bootstrap-hostのIP変化)は今回のstale判定に含めない(確認済みの
  決定事項#1、単純な接続不能とネットワーク不調を区別できないため)。Epic Dの署名検証を
  復活させていない(sha256のみのポリシーは不変)。
