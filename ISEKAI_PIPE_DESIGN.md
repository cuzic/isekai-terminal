...
### Epic Q / Epic Q-2: Claude Codeフック連携によるタブ状態表示(claude-hookd)— 独立リポジトリへ移設(2026-08-12)

`claude-hookd`(Claude Codeのフックイベントをターミナルタブの永続的な視覚
インジケータ〈背景色/OSC 9;4プログレスリング〉へ変換する常駐daemon)は、
2026-08-12に`rust-core/claude-hookd/`から**独立リポジトリ**
([cuzic/claude-hookd](https://github.com/cuzic/claude-hookd)、private)へ
分離した。以前ここに書かれていたEpic Q(実装済み設計)・Epic Q-2(delivery層の
プラグイン化検討)の詳細設計・意思決定の経緯は、分離元リポジトリの履歴として
このisekai-terminalリポジトリの`git log -- rust-core/isekai-pipe/src/claude_hookd
rust-core/claude-hookd`から、実装・現行ドキュメントは新リポジトリの
`README.md`から参照できる。

isekai-terminal側に残る関連情報:
- `$ISEKAI_CTL_SOCK`経由の`IsekaiPipeCtl`配送方式は、`isekai-ssh`が接続確立時に
  組み立てるctl-socket forward・`#@isekai tab-idle-color`/`tab-attention-color`
  ディレクティブ経由の`$ISEKAI_TAB_IDLE_COLOR`/`$ISEKAI_TAB_ATTENTION_COLOR`
  env var伝播を含め、`isekai-ssh`/`isekai-pipe`側は変更なくそのまま提供し続ける
  (`rust-core/isekai-ssh/src/ctl_forward.rs`・`rust-core/isekai-ssh/src/wrapper.rs`
  ・`rust-core/isekai-ssh/src/wrapper/config.rs`)。
- `isekai-pipe claude-hookd`サブコマンドは既に削除済み(`isekai-pipe/src/main.rs`)
  ——呼び出すと新リポジトリを案内するメッセージを出す。
