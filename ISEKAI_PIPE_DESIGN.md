...
- ✅(部分)isekai-terminal本体アプリ(Android)側: `#4.3 isekai-terminal本体アプリ(Android/iOS)側は
  ...
  - **クリップボード(push方向、host→device)**: `terminal.rs`にOSC 52
    (`ESC]52;c;<base64>BEL`)のパースを追加(`pending_clipboard_write`、query
    `Pd=="?"`は非対応)。`ProcessResult`→`dispatch_result`経由で新設の
  ...
  - **未実装のまま(スコープ外として明示)**:
    - clipboard **pull**(device→host、OSC 52 query応答)。ホスト側から
      `ESC]52;c;?BEL`が来ても無視するのみ(`osc_dispatch`のquery分岐)。
    - Android `ClipData`の画像対応(`FileProvider`経由のcontent:// URI化、
      §「クリップボードは双方向・opt-in」参照)。OSC 52自体がテキストのみの
      プロトコルであるため、画像はそもそも新設の`isekai-pipe ctl`ワイヤー
      (`CtlMessage::ClipboardPush`)経由でしか運べない。
    - isekai-terminal本体アプリ向けの「tmuxを迂回する側チャンネル」
      (isekai-sshの`ctl-socket` forwardに相当するもの)は未着手。本体アプリは
      自前のSSHクライアント(russh)を持つため、`-R`のような外部sshの仕掛けに
      頼らず独自チャンネル(例: 追加のSSHチャンネル種別)で実現できるはずだが、
      russh側のAPI調査を要するため別タスクとする。現状は「tmux側で
      `set-titles`/`allow-passthrough`が効いていれば動く」標準OSC経路のみ。
    - iOS側は未着手(Android側の実装を踏まえて後日移植)。
