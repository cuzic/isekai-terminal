...
実際の資源保護は`SessionTable::insert_existing`の容量ベースLRU立ち退き(`--max-sessions`
超過時に`parked_since`最古のセッションを立ち退かせる)が一次防御として機能している。

resume window超過時、および`RESUME`が`UnknownSession`(サーバがそのセッションを既に
知らないという確定的な signal)で拒否された時点で、クライアントは即座に諦める
——後者はresume windowの`deadline`を待たない(`isekai-pipe/src/resume_loop.rs::
resume_with_backoff_until_deadline`)。resume windowが数日オーダーになった今、
サーバプロセス再起動のような「絶対に成功しない」ケースをdeadlineまで機械的にリトライ
し続けると、即座に診断できたはずの失敗が数日間のハングになってしまうため。
...
- ✅ `isekai-ssh`ラッパー側のper-tab UDS forward(`#@isekai ctl-socket yes`、opt-in・
  既定`no`)。当初案から2点変更(下記「設計」参照): リモートパスは`~/.cache/...`ではなく
  `/tmp`直下+128bit乱数トークン、環境変数は`SetEnv`ではなくリモートコマンドのラップで渡す。
  クライアント側(`isekai-ssh::ctl_forward.rs`)はUnixで`UnixListener`、Windowsではローカル
  ループバック`TcpListener`(`127.0.0.1`固定ポート、Win32-OpenSSHがWindows名前付きパイプ/
  AF_UNIXを`-R`/`-L`でサポートしないための代替)の両方を実装済み(Windows公式サポート化、
  2026-07)——「黙って無効化」ではない。明示的なリモートコマンドを指定した呼び出しでは
  両プラットフォームとも黙って無効化(対話シェルのみが対象、この判断自体は変更なし)。
  リモート側(`isekai-pipe::ctl.rs`、`isekai-pipe ctl`サブコマンド)は常にリモートLinuxホスト
...
- ✅(部分)isekai-terminal本体アプリ(Android)側: `#4.3 isekai-terminal本体アプリ(Android/iOS)側は
  ...
  - **タイトル**: 追加の配線は不要だった——`ScreenUpdate.title`は既に
    `TerminalSession.state`まで届いていた(未使用のまま)。`TerminalHostScreen.kt`の
    タブバーが`uiState.screenUpdate?.title`を優先表示し、無ければ`tab.label`
    (プロファイル名)にフォールバックするようKotlin側だけ変更。
  - **クリップボード(push方向、host→device)**: `terminal.rs`にOSC 52
    (`ESC]52;c;<base64>BEL`)のパースを追加(`pending_clipboard_write`)。
    `ProcessResult`→`dispatch_result`経由で新設の
    `OrchestratorCallback::on_clipboard_write(text)`/`SessionCallback::on_clipboard_write(text)`
    を呼ぶ(UniFFIバインディング再生成・`./gradlew compileDebugKotlin`で確認済み)。
    Android側は`SharedPreferences("isekai_terminal_ui")`の
    `allow_remote_clipboard_write`(既定false、opt-in、`ProfileListScreen.kt`の
    メニューでトグル)を`TerminalTabsViewModel`が`TerminalSession`生成時に読み、
    有効なら`ClipboardManager.setPrimaryClip(ClipData.newPlainText(...))`する。
  - **クリップボード(pull方向、device→host、OSC 52経路)**: `terminal.rs`が
    query(`ESC]52;c;?BEL`)を検出すると`pending_clipboard_pull_request`を立てる
    (実際のAndroidクリップボード取得は非同期のKotlin往復が要るため、同期的な
    VTEコールバック内では完結しない)。`session.rs`のイベントループが
    `dispatch_result`後に`tokio::task::spawn_blocking`で新設
    `on_clipboard_pull_request() -> Option<String>`(`on_host_key`/
    `on_agent_sign_request`と同じ同期ブロッキング呼び出し)を呼び、取得した
    テキストをbase64化してOSC 52応答を`TransportCommand::WriteStdin`経由で
    リモートへ送り返す。`None`(opt-in無効・クリップボード空)の場合は一切
    応答しない(機能の有無自体を教えない)。Android側は`allow_remote_clipboard_pull`
    (push側と別のopt-in、既定false)を`ProfileListScreen.kt`にpushと並べてトグル。
  - **tmux迂回チャンネル(russh streamlocal forward)**: isekai-sshの`ctl-socket`
    forward(`-R`相当)を、本体アプリの自前SSHクライアント(russh)で実現した。
    russh 0.48.2のクライアントAPI(`client::Handle::streamlocal_forward`/
    `cancel_streamlocal_forward`、`client::Handler::server_channel_open_forwarded_streamlocal`)
    が過不足なく揃っており、外部`ssh(1)`もisekai-pipeも経由しない。設計のポイント:
    - **識別**: `PooledSshHandle`/`RusshEventHandler`に`remote_forwards`
      (ポート→転送先)と同型の`ctl_forwards: Arc<Mutex<HashMap<String, Sender<CtlMessage>>>>`
      を追加し、パス文字列そのものをタブの識別子にした(`server_channel_open_
      forwarded_tcpip`が`connected_port`で経路表を引くのと同じパターン)。
      SSH接続プーリングで複数タブが同じ`Handle`を共有していても、各タブが
      `run_ssh_channel_loop`の中で自分専用のパスを生成・登録するため誤配送しない。
    - **命名規約**: リモートパスは`isekai-ssh`の`ctl_forward.rs`と同じ
      `/tmp/isekai-pipe-ctl-<128bit乱数hex>.sock`(GCスイープのprefixも共通)。
    - **ワイヤー**: `isekai_protocol::CtlMessage`をそのまま流用(isekai-pipeと
      型を共有)。`SetTitle`は`Terminal::set_title`(OSC経由を迂回する直接
      setter)+新設`SessionState::set_title_from_ctl`経由で`ScreenUpdate`に
      反映、`ClipboardPush`は既存の`on_clipboard_write`をそのまま呼ぶ
      (push側の実装を再利用)。
    - **有効化**: プロファイル単位ではなく**グローバル設定**(ユーザーとの合意)。
      `set_ctl_socket_forward_enabled`(`set_terminal_theme`と同じ「Kotlin起動時に
...
- タブ(=`isekai-ssh <hostname>`の1回の起動)ごとに、一意なリモートパスを持つ
  **UNIX domain socket**を`-R`でローカルへ転送する(`-R <remote-sock>:<local-sock>`)。
  ...
  - リモート側パス(**実装時に確定**): `/tmp/isekai-pipe-ctl-<128bit乱数hex>.sock`。
    当初案の`~/.cache/isekai-pipe/ctl/<uuid>.sock`から変更した。理由は`-R`の
    リモート側パスは`sshd`にそのまま渡され、tilde展開されない(シェルを経由しない)ため、
    リモートの`$HOME`を先に知る必要がある(=接続のたびに追加のラウンドトリップが要る)。
    Epic Lで避けてきた「毎接続の追加コスト」を新たに持ち込むより、128bitのランダム
    トークンをファイル名にすることで衝突・先取り耐性を確保する方針にした(`bind()`は
    既存パスへは失敗するため、攻撃者はトークンを事前に言い当てない限り横取りできない)。
    実sshd越しに`-R /tmp/...:...`のstreamlocal forward自体が想定通り動くことは
    手動で実機検証済み(tilde展開なしで正しくバイト列が往復することを確認)。
  - ローカル側パス: `isekai-ssh`ラッパー自身が生成・listenする一意なUNIX domain socket
    (`default_runtime_dir()/ctl/<token>.sock`、既存の`ISEKAI_PIPE_RUNTIME_DIR`/
    `XDG_RUNTIME_DIR`解決を再利用、ディレクトリは0700)。
  ...
  - **スコープ限定(実装時に確定)**: 明示的なリモートコマンドを指定した呼び出し
    (`isekai-ssh host 'some command'`)は対象外(対話シェルが無いと`isekai-pipe ctl`を
    手で叩けないため、機能を足す意味がない)。ローカル側リスナーは`tokio::net::UnixListener`
    にWindowsバックエンドが無いため、Windows公式サポート化(2026-07)でループバック
    `TcpListener`(`127.0.0.1`固定ポート)による代替実装を追加した(`isekai-ssh::ctl_forward.rs`、
    上のEpic M実装状況の項目も参照)——もはや「`#[cfg(unix)]`のみ」ではない。どちらの
    有効化条件も満たさない場合は接続自体は失敗させず黙ってforwardなしで続行する
    (opportunistic fallback、この判断自体は変更なし)。
- リモートの対話シェルには`ISEKAI_CTL_SOCK=<remote-sock>`を環境変数として渡す
  (**実装時に確定**: `-o SetEnv=...`ではなく、明示的なリモートコマンドが無い呼び出し
  (上記のスコープ限定によりこの経路にしか到達しない)に対して
  `export ISEKAI_CTL_SOCK=<remote-sock>; exec "${SHELL:-/bin/sh}" -i -l`を
  リモートコマンドとして注入する形にした。`SetEnv`は受け側`sshd_config`の`AcceptEnv`/
  `SetEnv`設定が要り、多くのユーザーが変更できないリモートの設定に依存してしまうため
  避けた。この方式は`sshd`がリモートコマンド無し接続に対して行う「login shellの暗黙
  exec」を明示的に模倣する形になる——実sshd越しにこの`export ...; <rest>`という
  ラップがexportした環境変数を確実に`<rest>`へ渡すことも実機検証済み)。
...
- **既知の制限**: 別のSSH接続時に開始・attachされたtmuxセッション(reconnect跨ぎ)では
  ...
  フォールバックする」方針に合わせる)。
- **stale UNIX domain socketのGC(実装済み)**: セッションが異常切断(クラッシュ・
  `kill -9`・ネットワーク瞬断など、正常な`ssh -O cancel`+unlinkのパスを通らない終了)
  した場合、socketファイルが残留しうる。常駐監視プロセスは置かず、**次に同種のsocketを
  作るタイミングでのlazy sweep**だけで対処する(`isekai-pipe-core::sweep_stale_sockets`、
  isekai-ssh・isekai-pipe両方から共有)。
  - ディレクトリ内の該当prefixの`*.sock`それぞれに`connect()`を試みる(UNIX domain
    socketの`connect()`はネットワークラウンドトリップが無いため、非blocking化は不要)。
    `ECONNREFUSED`なら即`unlink`する。**実装時に判明した点**: Linux上では、そのパスが
    そもそもsocketではない普通のファイル(壊れた残骸)であっても`connect()`は
    `ENOTSOCK`ではなく同じ`ECONNREFUSED`を返す——つまり「見捨てられたsocket」と
    「socketですらない残骸」は`connect()`単体で両方とも検知できる。
  - mtime一定期間超過(既定24時間)によるフォールバックも残しているが、上記の理由で
    実際にこちらが効くのは「`connect()`の結果が`ECONNREFUSED`以外の何か曖昧なエラー
    (例: 権限エラー)を返すレアケース」に限られる。
  - ローカル側(`isekai-ssh`: `default_runtime_dir()/ctl/`)はディレクトリ自体が
    (`$XDG_RUNTIME_DIR`ベースの場合)ログアウト/再起動でtmpfsごと消えるため、長時間
    ログインしっぱなしのセッションでクラッシュが積み重なるケースだけがlazy sweepの
    実質的な対象になる。タブが`ctl-socket`を新しく張る直前(`prepare_ctl_forward`)に
    毎回スイープする。
  - リモート側(`/tmp/isekai-pipe-ctl-*.sock`)の実際のsocket bind自体は`-R`の
    streamlocal forwardを持つ`sshd`が管理しており、正常なchannel/接続終了時は`sshd`
    自身がunlinkする(こちらが管理する必要はない)。異常終了時に残ったファイルは、
    次回`isekai-pipe serve`起動(bootstrap)のタイミングでスイープする。
...
### Epic N: stale trustの自動検知・自動再bootstrap — 完了(2026-07-10)

**動機**: `isekai-pipe serve`(デプロイ済みのisekai-helper)は`session_secret`とTLS証明書を
**起動のたびに毎回ランダム生成**し、永続化しない(`isekai-pipe/src/engine/mod.rs`)。その
ためデプロイ済みプロセスが一度でも再起動(クラッシュ・ホスト再起動・手動再起動)すると、
信頼ストアに書いてある`cached_session_secret`/`cached_cert_sha256`は即座に無効になる。
しかし`PersistentProfile`のレコード自体は存在し続けるので、`wrapper.rs::build_connection_intent`
は成功してしまい、失敗は`ssh`のProxyCommand内(`isekai-pipe connect`)の実際のQUIC認証
段階まで表面化しない。従来これを自動検知・自動復旧する仕組みは無く、ユーザーが気づいて
手動で`isekai-ssh init`を再実行するしかなかった。

ユーザー要望: 「`isekai-ssh <hostname>`と打てば、どういう状態だったとしても内部的に
適切に判定してつながる」UIにしたい。stale trustを検知したら自動的にbootstrapをやり直す。

**確認済みの決定事項**(ユーザーに直接確認):
1. stale判定の範囲は**狭い範囲**: 証明書pin不一致 / session_secret認証拒否のみ。単純な
   接続タイムアウト/refusedは対象外(ネットワーク不調と区別できないため、無駄なSSH
   再デプロイを避ける)。
2. 既に信頼済みのホストを自動で再bootstrapする際、`[y/N]`のTOFU確認プロンプトは
   **常に無条件でスキップ**する(再デプロイ自体は引き続きユーザーの`~/.ssh/known_hosts`
   によるホスト鍵検証を経由するため、既存の信頼済みプロファイルの単なる更新として扱う)。
   新規(未登録)ホストの初回確認は従来通り必須のまま変更していない。

- ✅ **失敗分類(isekai-transport)**: `TransportError::CertPinMismatch { expected, got }`を
  追加。`PinnedCertVerifier`(rustlsの`ServerCertVerifier`トレイトの戻り値型に縛られ型付き
  エラーを直接返せない)は`Arc<Mutex<Option<(String,String)>>>`スロット経由でTLSの挙動は
  変えずに理由だけ副チャンネルで復元する。`TransportError::is_stale_trust_signal()`
  (`CertPinMismatch`と`Rejected(AttachRejectReason::Auth)`の2つだけ`true`)を追加し、
  `AttemptFailure`/`SequentialConnectError`/`SequentialStunConnectError`にも同名の委譲
  メソッドを追加(`isekai-bootstrap-plan::BootstrapFailure`の`may_retry`等と同じ
  「predicateメソッドを型に生やす」慣習を踏襲)。`isekai-transport::StaleTrustSignal`
  (マーカー型)を`isekai-pipe/src/main.rs`の4箇所(型付きエラー→anyhow変換地点)で
  `.context()`付与する(`BootstrapFailure`の「attach at the source, downcast at the top」
  パターンと同じ形)。
- ✅ **isekai-pipe connect → isekai-ssh wrapperへの通知(ファイルベース側路)**:
  `isekai-pipe-core::outcome`モジュール新設。既存の`ConnectionIntent`のatomic write /
  claim-by-renameパターンをそのまま踏襲し、`<runtime_dir>/connect-outcomes/<intent_id>.json`
  に`ConnectOutcome{schema_version, intent_id, profile, class: StaleTrust, detail}`を書く。
  `connect_command`は`StaleTrustSignal`をdowncastできた場合のみ、`ISEKAI_INTENT_ID`が
  設定されている時(=wrapper経由起動時のみ)書き込む。書き込み失敗はログして無視し、
  終了コード・stdout(stdout purity)には一切影響させない。
- ✅ **wrapperの再試行ロジック**: `wrapper.rs::run()`の末尾を`run_ssh_once`として抽出。
  `run_ssh_with_stale_trust_recovery`: 1回目のssh起動が失敗し`claim_connect_outcome`で
  stale信号が見つかった場合、`should_bootstrap()`によるopt-out確認(`--isekai-no-bootstrap`
  等が引き続きそのまま効く、新しいdirectiveは追加していない)の上で自動的に再bootstrapし
  (`bootstrap_and_register`に`TofuConfirmation::Silent`を渡し`[y/N]`確認を無条件で
  スキップ)、2回目のssh起動を行う。ループ・再帰は使わず最大2回のssh起動という構造で
  無限ループを構造的に排除している。stale信号が無い場合は今まで通りそのまま失敗を返す。
- ✅ **`isekai-ssh doctor <host> [--fix] [--helper-binary PATH] [--stun-server ADDR]`
  サブコマンド**: 手動診断用。`isekai-ssh <host>`の通常経路からは一切呼ばれない(自動復旧は
  wrapper.rs内で完結している)。`wrapper::resolve_profile_for_destination`(config解決の
  再利用)と`bootstrap_and_register(..., TofuConfirmation::Silent)`(`--fix`時の再利用)で
  重複実装を避けている。`isekai-pipe probe --json`をサブプロセスとして呼び出し結果を要約
  表示する(`ProbeReport`を共有ライブラリ化する道もあったが、doctorは毎接続のホットパス
  ではない occasional な手動診断なので、プロセス起動コストを許容してこちらを選んだ)。
  `isekai-pipe probe`側には`ProbeReport.stale_trust_suspected`フィールドを追加。
  `wrapper::RESERVED_SUBCOMMANDS`(旧`LEGACY_SUBCOMMANDS`)に`"doctor"`を追加し
  `should_run_wrapper`がホスト名と誤解釈しないようにした。
- ✅ テスト: `isekai-transport`の`is_stale_trust_signal()`各型での網羅的unit test。
  `isekai-pipe-core::outcome`のwrite/claim round-tripテスト。`isekai-pipe`の新規e2e
  (`stale_trust_signal_e2e.rs`): 実際に動作中の`isekai-pipe serve`にわざと誤った
  `cached_session_secret`を持つプロファイルを事前登録し、stdout空・exit code 69・
  `connect-outcomes/`にファイルが書かれることを確認(単純な接続不能では書かれないことも
  確認、分類の狭さの担保)。`isekai-ssh`の新規e2e(`wrapper_stale_trust_auto_recovery_e2e.rs`):
  1回目接続失敗→無確認での自動再bootstrap→プロファイル更新、という一連の流れをE2Eで検証
  (`--isekai-no-bootstrap`指定時は再bootstrapが起きないことも確認)。`doctor_e2e.rs`:
  未bootstrap・stale(`--fix`なし/あり)の3パターン。
- **スコープ外(意図的に対象外、当初版)**: QUICレベルのresume/multipath/rebind自体の変更なし
  (初回attach段階の分類のみ)。新規ホストのTOFU確認は変更なし(必須のまま)。新しい
  `#@isekai`ディレクティブは追加していない。Epic Dの署名検証を復活させていない
  (sha256のみのポリシーは不変)。
  当初は「キャッシュされたアドレス自体の古さ(STUN観測アドレス・direct-by-bootstrap-host
  のIP変化、単純な接続タイムアウト/refused)」もstale判定に含めない方針だった
  (単純な接続不能とネットワーク不調を区別できないため、無駄なSSH再デプロイを避ける
  という判断)——**この判断は下記「Epic N-2」で覆されている**。

#### Epic N-2: 「常に接続できる」原則への拡張 — 完了(2026-07-11)

**動機**: Epic Nの「単純な接続タイムアウトは対象外」という判断が、実際に運用上の
致命的な問題を引き起こした——`isekai-pipe serve`プロセスが(WiFi切断時のクライアント
側resume失敗、手動kill、ホスト再起動等で)死んでいる状態でキャッシュされたエンドポイント
に再接続しようとすると、QUIC接続が単純にidle timeoutするだけで`StaleTrust`に分類されず、
`isekai-ssh`は自動復旧を一切試みずにそのまま失敗して終了していた。ユーザーは`isekai-ssh
doctor --fix`または`isekai-ssh init`を**手動で**実行しない限り、何度`ssh`を再実行しても
永久に同じ失敗を繰り返す状態に陥っていた。

**確定した設計原則(最優先)**: **`isekai-ssh <hostname>`は、ローカルにキャッシュされた
デプロイ情報がどんな状態(古い・死んでいる・fingerprint不一致・プロセスがkillされた等)
であっても、実際のネットワーク経路とSSHアクセス自体が生きている限り、自動的に(サイレント
再デプロイを挟んで)接続できなければならない。** ユーザーが`isekai-ssh doctor --fix`/`init`
を手動で実行しなければ復旧しない状態は、原則としてバグとして扱う。唯一の例外は、本質的に
自動化できないケース(新規ホストの初回TOFU確認、`isekai-ssh login`のトークン失効等の
ユーザー入力が必要な場合)。

- ✅ **`ConnectOutcomeClass`に`Unreachable`を追加**(`isekai-pipe-core/src/outcome.rs`):
  `StaleTrust`(証明書pin不一致/Auth拒否、Epic Nの狭い分類のまま維持)と並ぶ、それ以外の
  **あらゆる`run_connect`失敗**(単純なQUIC idle timeout、fencing拒否、resume失敗等)を
  表すクラス。`connect_command`(`isekai-pipe/src/main.rs::write_connect_outcome_for_wrapper`)
  は`run_connect`が失敗するたびに必ずどちらかのクラスで`ConnectOutcome`を書くようになった
  (以前は`StaleTrustSignal`をdowncastできた場合のみ書いていた)。`run_connect`の失敗は
  常にSSHバイトが一切流れる前に起きる(これは`ssh`のProxyCommand自体であり、リモート
  シェルコマンドの終了コードとは無関係)ため、「`run_connect`が失敗したかどうか」自体が
  安全で一般的なトリガーになる。
- ✅ **`isekai-ssh`側の再試行ロジックを一般化**(`wrapper.rs`): `run_ssh_with_stale_trust_recovery`
  → `run_ssh_with_connect_failure_recovery`、`StaleTrustRecoveryAction` →
  `ConnectFailureRecoveryAction`に改名。`decide_connect_failure_recovery`は`ConnectOutcome`
  の有無だけを見る(クラスを問わない)——両クラスとも全く同じ「サイレント再bootstrap+
  1回だけ再試行」を発動する。ユーザー向けメッセージだけ`outcome_summary()`でクラスごとに
  出し分ける("cached trust looks stale" vs "the cached deployment could not be reached")。
  再試行の構造(1呼び出しにつき最大2回のssh起動、ループ・再帰なし)自体は不変。
- ✅ **`isekai-pipe/src/engine/resume.rs`の関連バグを同時に修正**: `SessionTable::
  sweep_expired_parked`(park期限切れ)と`insert_existing`(テーブル満杯時の立ち退き)の
  両方が、破棄/立ち退きさせたsessionの`AttachArbiter`側`Established` fencing slotを
  解放していなかった(`SessionTable`単体からは`AttachArbiter`に触れられない)。どちらも
  修正済み(`isekai-pipe serve`プロセスを再起動しない限り対象ターゲットへの新規ATTACHが
  永久に`BUSY_OTHER_SESSION`で拒否され続けるバグだった——サーバ側状態はクライアント側の
  再試行では原理的に回復不可能なため、これも「常に接続できる」原則の一部として同時に潰した)。
- ✅ テスト: `isekai-pipe-core::outcome`に`Unreachable`のround-tripテスト追加。
  `isekai-pipe`の`stale_trust_signal_e2e.rs`を更新——「単純な接続不能では書かれない」
  テストを「`Unreachable`クラスで書かれる」テストに置き換え。`isekai-ssh`の
  `wrapper_stale_trust_auto_recovery_e2e.rs`に
  `wrapper_silently_recovers_from_an_unreachable_cached_endpoint_and_reconnects`を追加
  (修正前のコードに対して実行し、実際に自動復旧が起きず失敗することを確認済み)。
  `isekai-pipe`の`serve_e2e.rs`に`fresh_attach_after_park_expiry_succeeds_instead_of_staying_busy`
  を追加(sweep側のfencing leakの回帰テスト、同様に修正前コードでの失敗を確認済み)。
- **スコープ外(Epic N-2でも変更していない)**: `is_stale_trust_signal()`自体の分類は
  従来通り狭いまま(`CertPinMismatch`/`Rejected(Auth)`のみ)——これは「候補フォールバック
...
- ✅ **`UnknownSession`は即座にgive up**(回帰防止): resume windowが数日オーダーに
  伸びたことで、サーバプロセスの再起動のような「そのsession_idでは絶対に成功しない」
  ケースをdeadlineまで機械的にリトライし続けると、即座に診断できたはずの失敗が
  数日間のハングになってしまう。`isekai-pipe/src/resume_loop.rs::
  resume_with_backoff_until_deadline`に、`TransportError::ResumeRejected
  (ResumeRejectReason::UnknownSession)`(サーバがそのセッションを既に知らないという
  確定的signal)を検出したら即座にgive upする分岐を追加した(deadline超過時と同じ
  give-up処理を共通の`give_up`ヘルパーへ切り出して共有)。
- この設計変更はCodex CLI(`codex exec -s read-only`)にセカンドオピニオンとして
  レビューさせた——helper再利用fingerprintの見落とし・`UnknownSession`即時give-upの
  必要性は、いずれもそのレビューで指摘されて設計に取り込んだもの。
- **スコープ外**: parkされたセッションは`AttachState::Established`のままなので
  `is_vacant()`がfalseになり、`--max-idle-lifetime`の自己終了ウォッチドッグが
  発火しない(既存の許容されているtrade-offの延長で、今回のデフォルト変更により
  「park 1つがhelper自身を最大10日生かし続ける」規模になった)。今回はこの相互作用を
  明文化するに留め、動作は変えていない。Android側(`rust-core/src/
  multipath_transport.rs`)は`requested_resume_grace_secs = 0`固定(サーバ既定値に
  追従)のため変更不要——サーバ既定値の引き上げで自動的に恩恵を受ける。
...
公式サポート対象外」と明記していたが、実装(`cfg(windows)`分岐・`ctl_forward.rs`のTCP
ループバック実装等)は既にこの宣言より進んでおり、記述が実態と乖離していた。README宣言を
実態に合わせて更新するため、残っていた実質的なギャップ(セキュリティ上のno-op・HOME解決の
不整合・e2eテスト未対応)を埋めた。接続先リモートホストが常にLinux固定という既存方針
(`PLAN.md` 602行目)には一切手を入れていない——クライアント(`isekai-ssh`実行環境)としての
Windows対応のみが対象。

- ✅ **HOME/USERPROFILE解決の統一**: `isekai-ssh/src/wrapper.rs`の`config_roots`/
  `expand_path`・`helper_download.rs`の`default_helper_cache_dir`が生の`std::env::var_os("HOME")`
  のみを見ており、`isekai-trust`/`isekai-auth`が既に使っていた`isekai_fs_guard::resolve_home_dir()`
  (`HOME`→`USERPROFILE`フォールバック)を使っていなかったのを統一した。
- ✅ **`isekai-fs-guard`のWindows ACL実装**: `check_not_world_writable`/
  `set_private_file_permissions`/`ensure_private_dir`がWindowsで無条件successのno-opで、
  トラストストア・トークンファイルの「他ユーザー書き込み不可」保証が事実上機能していなかった
...
- ✅ **`isekai-ssh`のmock sshdベースe2eテスト5本のWindows対応**
  (`wrapper_auto_bootstrap_e2e.rs`/`doctor_e2e.rs`/`init_e2e.rs`/
  `wrapper_auto_download_relay_bootstrap_e2e.rs`/`wrapper_stale_trust_auto_recovery_e2e.rs`)。
  実`ssh(1)`の代わりに使うシムを、Windowsでは`.cmd`バッチファイルではなくコンパイル済み
  `.exe`(`src/bin/ssh_test_shim.rs`、環境変数経由で実sshパス/`-F`設定ファイルを受け取り
  そのまま中継する)にした——`.cmd`はstd::process::CommandのWindowsバッチ引数安全性検証
  (CVE-2024-24576対策)により改行を含む引数(実際のbootstrap deployのリモートコマンド)を
  拒否されるため。副次的に見つかったproduction側のバグも修正: `bootstrap_and_register`/
  `doctor`/`init`が`--isekai-ssh-path`をbootstrap用の`OpenSshBackend`へ伝播しておらず、
  `OpenSshBackend::new()`の既定値(裸の`"ssh"`)を常に使ってしまっていた不整合(`doctor`/
  `init`には`--ssh-path`フラグを新設)。
- ✅ **`test-windows` CI job**(`rust-core-test-check.yml`)に上記5ファイルを個別に明示実行
  するステップを追加。ワークスペース全体への`--tests`一括拡張はしていない(接続先が常に
  Linuxである前提そのものを検証する`real_sshd_*`系・`isekai-bootstrap/tests/openssh_e2e.rs`
  等のUnix専用統合テストを誤って巻き込まないため)。
- **スコープ外(意図的)**: trust store・token・helper cacheの保存先パスは
  `resolve_home_dir()`ベースのXDG風パスのままで、`isekai_pipe_core::default_profiles_dir()`
  が使う`%LOCALAPPDATA%`優先方式には統一していない(動作はするが非イディオマティック、
  README.mdに明記)。`real_sshd_*`系e2eテスト・`isekai-bootstrap/tests/openssh_e2e.rs`は
  Linux/macOS限定のまま(接続先が常にLinuxである前提を検証するテストであり、
  isekai-sshクライアント自身のWindows対応とは無関係)。

**参照実装**:
- `rust-core/isekai-fs-guard/src/windows_acl.rs`
- `rust-core/isekai-ssh/src/bin/ssh_test_shim.rs`
- `rust-core/isekai-ssh/src/wrapper.rs`: `resolve_windows_executable`/`is_posix_shell_ssh`/
  `proxy_command`/`quote_proxy_command_path`(いずれもEpic O以前から存在、README宣言との
  乖離の一因だった実装)
