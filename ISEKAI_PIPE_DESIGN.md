...
- ✅ `isekai-ssh`ラッパー側のper-tab UDS forward(`#@isekai ctl-socket yes`、opt-in・
  ...
  `#[cfg(unix)]`のみ(Windowsでは黙って無効化)。明示的なリモートコマンドを指定した
  呼び出しでも黙って無効化(対話シェルのみが対象)。
...
- タブ(=`isekai-ssh <hostname>`の1回の起動)ごとに、一意なリモートパスを持つ
  **UNIX domain socket**を`-R`でローカルへ転送する(`-R <remote-sock>:<local-sock>`)。
  ...
  - **スコープ限定(実装時に確定)**: 明示的なリモートコマンドを指定した呼び出し
    ...
    手で叩けないため、機能を足す意味がない)。`#[cfg(unix)]`のみ(`tokio::net::
    UnixListener`にWindowsバックエンドが無いため)。どちらも有効化条件を満たさない
    場合は接続自体は失敗させず黙ってforwardなしで続行する(opportunistic fallback)。
